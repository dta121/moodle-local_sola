 ...
 *
 * Provides a simplified non-streaming chat interface for the Moodle mobile app.
 *
...
class mobile {
    ...
    /**
     * Return the mobile view for the course chat entry.
     *
    ...
    /**
     * Return the Angular/Ionic mobile template.
     *
...
HTML;
    ...
    /**
     * Return the JavaScript controller for the mobile chat.
     *
     * @return string
     */
...
var cleanAssistantText = function(text) {
    return (text || '')
        .replace(NEXT_RE, '')
        .replace(SOURCE_RE, '')
        .trim();
...
that.loadHistory = function() {
    that.CoreSitesProvider.getCurrentSite().read(
        ...
        if (result && result.messages) {
            that.messages = result.messages.map(function(message) {
                var text = message.role === 'assistant'
                    ? cleanAssistantText(message.message)
                    : (message.message || '');
                return {
                    role: message.role,
                    message: text,
                    time: message.timecreated
                        ? new Date(message.timecreated * 1000).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit'
                        })
                        : ''
...
that.sendMessage = function() {
    ...
    that.CoreSitesProvider.getCurrentSite().write(
        ...
        {courseid: courseid, message: text}
    ).then(function(result) {
        if (result && result.response) {
            that.messages.push({
                role: 'assistant',
                message: cleanAssistantText(result.response),
                time: new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})
            });
        }
        that.sending = false;
